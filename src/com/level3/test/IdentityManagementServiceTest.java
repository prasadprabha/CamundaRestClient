package com.level3.test;

import org.junit.Test;
import org.junit.runner.RunWith;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.test.context.ContextConfiguration;
import org.springframework.test.context.junit4.SpringJUnit4ClassRunner;
import org.springframework.test.context.support.AnnotationConfigContextLoader;

import com.level3.rest.configuration.AppConfig;
import com.level3.rest.service.IdentityManagementService;

@RunWith(SpringJUnit4ClassRunner.class)
@ContextConfiguration(classes = AppConfig.class, loader = AnnotationConfigContextLoader.class)
public class IdentityManagementServiceTest {
	
	@Autowired
	IdentityManagementService identityManagementService;
	
	
	@Test
	public void testCreateGroup() {
		String groupId = "ccd1";
		String groupName = "ccd1";
		String groupType = "ccd1";
		identityManagementService.createGroup(groupId, groupName, groupType);
	}
	
	@Test
	public void testCreateUser() {
		String userId  = "prasad";
		String password = "prasad1234";
		String firstName = "Prasad"; 
		String lastName  = "Prabha";
		String email  = "prasadps@gmail.com";
		identityManagementService.createUser(userId, password, firstName, lastName, email);
	}
	
}